    ...
    _createClass(Splitter, [{
        ...
        value: function render() {
            ...
            if (this.props.isBorderTop || this.props.isBorderBottom) {
                var location = node.getParent().getLocation().getName();
                var barSize = node.getParent().getAttribute('barSize') || 25;
                var left = location === 'left' ? barSize : style.left;
                var width = location === 'left' ? parseFloat(style.left.split('px')[0]) - barSize : node.getParent().getTabHeaderRect().x - node.getRect().x;
                ...
                return _react2.default.createElement("div", { style: newStyle, className: "flexlayout__splitter" });
            } else {
